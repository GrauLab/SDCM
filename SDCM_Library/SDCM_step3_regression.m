...
% baseline signal for the detected signature.
  ...
  function [regressionResults, forPlots] = SDCM_step3_regression(signatureDefinition, inInfo)
    %% Initialize:
      ...
      %Initialize outputs:
        ...
        %For failsave logic for the special case that the signature focus becomes empty (see details in %2020-02: Failsave logic if the signature focus gets empty: below)
          bFailsaveTrigger4emptySignatureFocus = false;
          signatureDefinition.step3_regression.bFailsaveActive4emptySignatureFocus = false;
        ...
        %Copy order metrics to step3_regression (typically .signatureStrengths4G and .signatureStrengths4P), if not already present (i.e. if not the default signatureStrengths4* are configured):
  ...
  %                     ,2, inInfo.searchStrategy.correlationMoments, sL);
                    ...
                    bFailsaveTrigger4emptySignatureFocus = bFailsaveTrigger4emptySignatureFocus...
                      || sum(abs(signatureDefinition.step3_regression.signedExtendedW4G))==0 ...
                      || sum(abs(signatureDefinition.step3_regression.signedExtendedW4P))==0 ...
                    ;
                  ...
                  %Update the signature focus:
                    ...
                    bFailsaveTrigger4emptySignatureFocus = bFailsaveTrigger4emptySignatureFocus...
                      || sum(abs(signatureDefinition.step3_regression.signedFocusedW4G))==0 ...
                      || sum(abs(signatureDefinition.step3_regression.signedFocusedW4P))==0 ...
                    ;
          ...
          %2020-02: Failsave logic if the signature focus gets empty:
            if(~signatureDefinition.step3_regression.bFailsaveActive4emptySignatureFocus && bFailsaveTrigger4emptySignatureFocus)
              sMessage = SDCM_printStatus(0,['  <- FAILSAVE logic: Detected that the signature focus became empty after projecting the last regressed bimontonic curve onto step2_finalSignatureAxes, i.e. this curve is now in "empty high-dimensional space" without any well correlated data points. This usually means that we already were unable to find signature axes with a stable local maximum of the signature functional in step 2. Continuing would lead to a dissection of a zero matrix, re-detection of the same signature in the next iteration and an endless loop. Hence, we must dissect anything to bring the singal nearer towards zero.\n'...
               ...
               ,'This will now be tried to continue dissection of other signatures. The signature of this iteration should be reviewed with care!\n'...
              ]);
                warning(sMessage);
              %inactive/Reset step 2 to step 1:
                if(false) %keep step2 results, just disable bUpdateSignatureFocusInEachSignatureCurveRegressionIteration for failsave.
                  signatureDefinition.step2_finalSignatureAxes.R4G = signatureDefinition.step1_initialRepresentative.R4G;
                  signatureDefinition.step2_finalSignatureAxes.P4R4G = signatureDefinition.step1_initialRepresentative.P4R4G;
                  signatureDefinition.step2_finalSignatureAxes.signedFocusedW4G = signatureDefinition.step1_initialRepresentative.signedFocusedW4G;
                  signatureDefinition.step2_finalSignatureAxes.signedFocusedW4G_withPerpendicularSpace = signatureDefinition.step1_initialRepresentative.signedFocusedW4G_withPerpendicularSpace;
                  signatureDefinition.step2_finalSignatureAxes.signedExtendedW4G = signatureDefinition.step1_initialRepresentative.signedExtendedW4G;
                  signatureDefinition.step2_finalSignatureAxes.signatureSizeByCorrSum4G = signatureDefinition.step1_initialRepresentative.signatureSizeByCorrSum4G;

                  signatureDefinition.step2_finalSignatureAxes.R4P = signatureDefinition.step1_initialRepresentative.R4P;
                  signatureDefinition.step2_finalSignatureAxes.P4R4P = signatureDefinition.step1_initialRepresentative.P4R4P;
                  signatureDefinition.step2_finalSignatureAxes.signedFocusedW4P = signatureDefinition.step1_initialRepresentative.signedFocusedW4P;
                  signatureDefinition.step2_finalSignatureAxes.signedFocusedW4P_withPerpendicularSpace = signatureDefinition.step1_initialRepresentative.signedFocusedW4P_withPerpendicularSpace;
                  signatureDefinition.step2_finalSignatureAxes.signedExtendedW4P = signatureDefinition.step1_initialRepresentative.signedExtendedW4P;
                  signatureDefinition.step2_finalSignatureAxes.signatureSizeByCorrSum4P = signatureDefinition.step1_initialRepresentative.signatureSizeByCorrSum4P;

                  signatureDefinition.step2_finalSignatureAxes.log10_p4Correlations = signatureDefinition.step1_initialRepresentative.log10_p4Correlations;
                  signatureDefinition.step2_finalSignatureAxes.signatureSizeByCorrSum2D = signatureDefinition.step1_initialRepresentative.signatureSizeByCorrSum2D;
                else
                  assert(inInfo.dissection.bimonotonicRegression.bUpdateSignatureFocusInEachSignatureCurveRegressionIteration, 'inInfo.dissection.bimonotonicRegression.bUpdateSignatureFocusInEachSignatureCurveRegressionIteration was already disabled; hence, cannot disable it as failsave option');
                end
              %Reset step 3 outputs:
                signatureDefinition.step3_regression = struct(); %contains .eigenSI, .eigenSJ based on configured order metrics.
                %For failsave logic for the special case that the signature focus becomes empty (see details in %2020-02: Failsave logic if the signature focus gets empty: below)
                  bFailsaveTrigger4emptySignatureFocus = false;
                  signatureDefinition.step3_regression.bFailsaveActive4emptySignatureFocus = true;
                if(~isfield(signatureDefinition,'forPlots')) 
                  signatureDefinition.forPlots = struct(); 
                end
                %Copy needed fields form step 2:
              ...
              %Continue without adaptation in step 3:
                nRegressionIteration = 0;
                inInfo.dissection.bimonotonicRegression.bUpdateSignatureFocusInEachSignatureCurveRegressionIteration = false; %no iterative adaptation/projection of regressed curves on signature axes, just robust bimonotonic regression in the fixed step 1 signature focus until converged.
                  %<-i.e. signature strengths based on bimonotonic regression, corresponding ordering in equidistant signal strengths space and bimonotonic regression are iterated in a constant initially fixed signature focus. Convergence/oscillations not guaranteed, but after 7 convergence iterations it is accepted as is and we can proceed with dissection... (the signature focus can no longer become empty)
                continue;
            end
          ...
          %Check regression convergence:
            if(nRegressionIteration>1) %use all points before computing convergence break option.
              ...
              r4TopOfSignatureDissection = uncenteredWeightedCorrelation(... %nanmean for the first step where generalizedSignature.R4P ist still nan(1,nP)
                ...
                ...Option/Info 202002: can speed up with same point as above (ignore unstable dangling curve near zero) by using regressed signal strengths as another weight factor (r=0.28->0.95 in a problematic case): ,W2D(BTopSignatureMembers2D).*abs(max(signatureEigensignalOS_previous(BTopSignatureMembers2D),signatureEigensignalOS(BTopSignatureMembers2D))) ...
