    ...
    inInfo = mergeStructs(inInfo, defConfig);
      function defaultInfo = defaultConfig()
        ...
        %Main Signature definition plot:
          ...
          %Plot optics:
            defaultInfo.plots.minFigureHeight = 637;
...
%               ;
        ...
        %Create figure window:
          %Adaptive figure height logic:
            minFigureHeight = inInfo.plots.minFigureHeight;
            stdFigureHeight = 685;
          ...
          %Add color underbar and legend for sample classes if provided:
            ...
            if(bInPipelinePlot && isfield(inInfo.plots,'displayPredefinedClasses4aShifts')&&isfield(inInfo.plots.displayPredefinedClasses4aShifts,'samples'))
              ...
              for cil=1:length(inInfo.plots.displayPredefinedClasses4aShifts)
                ...
                if(~bMoveFullLegendsBelowTheHeatmaps) %old standard:
                  hLegend4SampleClasses = text(...
                     iif(mod(cil,2)==1,min(xRange)-abs(diff(xlim(aUnderbarTarget)))/20, max(xRange)+abs(diff(xlim(aUnderbarTarget)))/20) ...
                    ,mean(yRange)...
                ...
                if(bMoveFullLegendsBelowTheHeatmaps)
                  ...
                  hLegend4SampleClasses_abbrevOnly = text(...
                     ...iif(mod(cil,2)==0,min(xRange)-abs(diff(xlim(aUnderbarTarget)))/20, max(xRange)+abs(diff(xlim(aUnderbarTarget)))/20) ...
                     max(xRange)+abs(diff(xlim(aUnderbarTarget)))/20 ...
                    ,mean(yRange)...
                  ...
                  bOverlongLegend = length(uniqueSampleClasses)>4; %2024: hotfix for overlong legends: move them to the top so that labels become legible.
                  hLegend4SampleClasses = text(...
                     min(xRange) + (cil-1) * abs(diff(xlim(aUnderbarTarget)))/2.5 ...
                    ,iif(bOverlongLegend...
                      ,YLim(1) - abs(diff(YLim))*0.33 ... %2024: hotfix for overlong legends.
                      ,mean(YLim(2)+[1/500,21.7/500*iif(bInPipelinePlot,maxFigureHeight/figureHeight,1)]*abs(-YLim(1)+YLim(2))) + (0+2.5)*abs(diff(yRange)) ...  %default below the 4th subplot.
                     )...
